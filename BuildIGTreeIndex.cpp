...
#define FILE_NODES_GTREE_PATH "./map/NY/binary/NY.paths"
#define FILE_GTREE 			  "./map/NY/binary/NY.gtree"
#define FILE_ONTREE_MIND	  "./map/NY/binary/NY.minds"
...
unordered_map<int,int> graph_partition( set<int> &nset ){
	...
	int i = 0;
///////////
	ofstream out("./map/NY/binary/result.txt");
	out<<nset.size()<<endl;
	for ( set<int>::iterator it = nset.begin(); it != nset.end(); it++, i++ ){
		result[*it] = part[i];
//////////
...
///////
	FILE *fp;
    	if ((fp = fopen("./map/NY/binary/path.txt", "a")) == NULL)
        	printf("cannot open file!\n");	
	fprintf(fp,"dump node GTree path into file.\n");
///////
	for ( int i = 0; i < Nodes.size(); i++ ){
...
///////
		fprintf(fp,"   Node[%d] its GTree_path is [",i);
		for(int k=0;k<Nodes[i].gtreepath.size();k++)
		{
			fprintf(fp,"%d, ",Nodes[i].gtreepath[k]);
		}
		fprintf(fp,"]\n");
///////
	}
...
// FLAculate the distance matrix, algorithm shown in section 5.2 of paper
...
/*
	cout<<"treenodelevel.size()="<<treenodelevel.size()<<endl;
	for(int i=0;i<treenodelevel[0].size();i++)
	{
		cout<<treenodelevel[0][i]<<" ";
	}
	for(int i=0;i<treenodelevel[1].size();i++)
	{
		cout<<treenodelevel[1][i]<<" ";
	}
	cout<<endl;
*/
...
void hierarchy_shortest_path_FLAculation(){
	// level traversal
	vector< vector<int> > treenodelevel;
...
///////
	FILE *fp;
    	if ((fp = fopen("./map/NY/binary/mind.txt", "a")) == NULL)
        	printf("cannot open file!\n");	
	fprintf(fp,"dump distance matrix into file.\n");
///////
	for ( int i = 0; i < GTree.size(); i++ ){
		// union borders
///////
		
		fprintf(fp,"\n   GTree[%d] as follow and it is ",i);

		if(GTree[i].isleaf){
			fprintf(fp,"leafnode  {");
			for(int k=0;k<GTree[i].leafnodes.size();k++)
			{
				fprintf(fp,"%d,",GTree[i].leafnodes[k]);
			}
			fprintf(fp,"}");	
		}else
		{
			fprintf(fp,"not leafnode ");
			
		}
		fprintf(fp,"-----------------------and its father is G[%d] and its borders have %d\n",GTree[i].father,GTree[i].borders.size());
///////
		count = GTree[i].union_borders.size();
///////
		fprintf(fp,"   Count_union_borders=%d, [",count);	
		for(int k=0;k<count;k++){
			int s = GTree[i].union_borders[k];		
			fprintf(fp,"%d,",s);
		}	
///////		
		fwrite( &count, sizeof(int), 1, fout );
