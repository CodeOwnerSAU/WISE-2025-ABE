...
// ----- for pre query init, OCCURENCE LIST in paper -----
    ...
    vector<double> setIGTreeNodeKWDisMatrix();
    ...
    int buildGreedyPath(int startNode,int endNode,vector<int> POICandidate,vector<int> &Gpath);
    ...
    void GreedyFiler(int s, int t, int GreedyPath ,vector<int> &POICandidate, vector<int> &POICandidate_Greedy_Filter);
...
//    void expendGrid(Grid& grid,bitset<KIND> uncover,int rangeLength,vector<int> &currentGridList,vector<int> &newExpandGridList,int flag,vector<int> &newGridList);
    ...
    map<int, std::vector<int>> levelMap;
    int s,t;
    ...
    map<int, std::vector<int>> collectLeafByLevel();
};
#endif
